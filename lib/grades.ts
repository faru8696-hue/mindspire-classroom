...
//
// Correct / Wrong / Incomplete (for work that's only partway done, not
// wrong so much as unfinished). Older submissions may still carry
// 'partial', 'discussed', or 'needsmore' from before — those are
...
export type GradeKey = 'correct' | 'incorrect' | 'incomplete'
...
export const GRADE_LIST: GradeDef[] = [
  ...
  { value: 'incomplete', label: 'Incomplete',  icon: '…', solid: 'bg-gray-500 text-white',   badge: 'bg-gray-200 text-gray-700' },
]
...
export const GRADE_MAP: Record<string, GradeDef> = {
  ...
  partial:   { value: 'incomplete' as GradeKey, label: 'Partial Credit', icon: '½', solid: 'bg-amber-500 text-white', badge: 'bg-amber-100 text-amber-700' },
  discussed: { value: 'incorrect' as GradeKey, label: 'Discussed',      icon: '💬', solid: 'bg-blue-600 text-white',  badge: 'bg-blue-100 text-blue-700' },
  needsmore: { value: 'incomplete' as GradeKey, label: 'Partial Credit', icon: '½', solid: 'bg-amber-500 text-white', badge: 'bg-amber-100 text-amber-700' },
}
