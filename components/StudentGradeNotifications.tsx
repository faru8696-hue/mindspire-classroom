...
const GRADE_STYLE: Record<string, { icon: string; cls: string; label: string }> = {
  ...
  reminder: { icon: '📣', cls: 'border-indigo-300 bg-indigo-50', label: 'Reminder from your teacher' },
}
    ...
    <div className="mb-6">
      ...
      <div className="space-y-2">
        {notifs.map(n => {
          ...
          // Reminders are just a message — no "question" to caption
          // separately, so the message itself is the headline instead of
          // a generic title + a repeated "Reminder from your teacher" line.
          ...
          return (
            <Link key={n.id} href={n.href} className={`flex items-center gap-3 border-l-4 rounded-xl px-4 py-3 hover:opacity-80 transition-opacity ${style.cls} ${!n.read ? 'ring-2 ring-purple-200' : ''}`}>
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-gray-800 truncate">{isReminder ? (n.feedback ?? 'Reminder') : n.question_title}</p>
                <p className="text-xs text-gray-600">{isReminder ? style.label : `${style.label}${n.feedback ? ` — ${n.feedback}` : ''}`}</p>
              </div>
