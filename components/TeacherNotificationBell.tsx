  ...
  useEffect(() => {
    const channel = supabase.channel('teacher-notifications')
      .on('postgres_changes', {
        ...
        // Fetch student name + question title
        const [{ data: profile }, { data: question }] = await Promise.all([
          supabase.from('profiles').select('full_name').eq('id', row.student_id).single(),
          supabase.from('questions').select('title').eq('id', row.question_id).single(),
        ])
        const enriched = { ...row, student_name: profile?.full_name, question_title: question?.title }
          ...
          <div className="absolute right-0 top-10 w-80 bg-white rounded-xl shadow-xl border border-gray-200 z-50 overflow-hidden">
            ...
            <div className="max-h-80 overflow-y-auto">
              {notifications.length === 0 ? (
                ...
                <div
                  key={n.id}
                  className={`flex items-start gap-3 px-4 py-3 border-b border-gray-50 last:border-0 ${n.read ? '' : 'bg-purple-50/60'}`}
                >
                  <span className="text-lg flex-shrink-0">{n.type === 'help' ? '🙋' : '✓'}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-800">
                      {n.student_name ?? 'Student'}{' '}
                      <span className="font-normal text-gray-600">
                        {n.type === 'help' ? 'needs help' : 'is done'}
                      </span>
                    </p>
                    {n.question_title && (
                      <p className="text-xs text-gray-500 truncate">{n.question_title}</p>
                    )}
