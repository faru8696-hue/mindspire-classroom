...
export default async function DiagnosticQuestionsPage({
  ...
  const { data: questions } = await admin
    ...
    .select('id, topic_id, content, question_type, mcq_options, mcq_correct_index, image_url, source, explanation, answer_key, points, is_active')
    .eq('diagnostic_test_id', testId)
      ...
      <div className="space-y-2">
        {(questions ?? []).map(q => (
          <DiagnosticQuestionCard
            key={q.id}
            question={q as { id: string; topic_id: string; content: string; question_type: 'mcq' | 'frq'; mcq_options: string[] | null; mcq_correct_index: number | null; image_url: string | null; source: string | null; explanation: string | null; answer_key: string | null; points: number | null; is_active: boolean }}
            topics={topics ?? []}
